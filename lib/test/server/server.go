...
func New(startConfig config.Config, keycloakExportLocation string) (config config.Config, shutdown func(), err error) {
	config = startConfig
	...
	go func() {
		...
		closer, err := Kafka(pool, zookeeperUrl)
		mux.Lock()
		defer mux.Unlock()
		closerList = append(closerList, closer)
		if err != nil {
	...
	go func() {
		...
		closer, _, permIp, err := PermSearch(pool, zookeeperUrl, elasticIp)
		mux.Lock()
		defer mux.Unlock()
		permissionUrl = "http://" + permIp + ":8080"
		config.SourceListUrl = permissionUrl
		closerList = append(closerList, closer)
		if err != nil {
			globalError = err
			return
	...
	go func() {
		...
		closer, _, ip, err := DeviceRepo(pool, mongoIp, zookeeperUrl, permissionUrl)
		mux.Lock()
		defer mux.Unlock()
		deviceRepoUrl = "http://" + ip + ":8080"
		closerList = append(closerList, closer)
		if err != nil {
			globalError = err
			return
	...
	go func() {
		...
		closer, _, ip, err := DeviceManager(pool, zookeeperUrl, deviceRepoUrl, "-", permissionUrl)
		mux.Lock()
		defer mux.Unlock()
	...
	go func() {
		...
		mux.Lock()
		defer mux.Unlock()
		config.AuthUrl = "http://" + ip + ":8080"
		//config.AuthClientSecret = "d61daec4-40d6-4d3e-98c9-f3b515696fc1"
		config.AuthClient = "frontend"
		closerList = append(closerList, closer)
		if err != nil {
			globalError = err
			return
		}
		err = ConfigKeycloak(config.AuthUrl, keycloakExportLocation, config.AuthClient)
		if err != nil {
